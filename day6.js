...
// # problem1
function getAnswers(groupAsks) {
  const map = {};
  for (let i = 0; i < groupAsks.length; i++) {
    const text = groupAsks[i];
...
for(let i = 0; i < inputArr.length; i++) {
  const group = inputArr[i];
  answer += getAnswers(group);
}
console.log('problem1 answer', answer);

// # problem2
function getAnswers2(groupAsks) {
  const map = {};
  for (let i = 0; i < groupAsks.length; i++) {
    const text = groupAsks[i];
    for(let j = 0; j < text.length; j++) {
  ...
  return Object.keys(map).filter(key => map[key] === groupAsks.length).length;
}

answer = 0;
for(let i = 0; i < inputArr.length; i++) {
  const group = inputArr[i];
  answer += getAnswers2(group);
}
