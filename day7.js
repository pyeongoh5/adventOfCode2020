let inputs = `plaid magenta bags contain 2 clear lavender bags, 3 clear teal bags, 4 vibrant gold bags.
...
class Bag {
  ...
  addParent(parentBag) {
    console.log('exist', parentBag.name, !this.isExist(parentBag));
    this.parent.push(parentBag);
  }
  addChild(childBag) {
    this.children.push(childBag);
  }

  isExist(parentBag) {
    return this.parent.some((p) => p.name === parentBag.name);
  }

  getParentRemoveDuplicated() {
    const parentMap = {}
    ...
    return Object.keys(parentMap).map((key) => parentMap[key]);
  }

  getChildRemoveDuplicated() {
    const childMap = {}
    ...
    return Object.keys(childMap).map((key) => childMap[key]);
  }
}
...
for(let i = 0; i < input.length; i++) {
  const bagRelation = input[i];
  const [container, args] = bagRelation.split(' bags contain');
  const bags = args.split(',').filter((bagName) => !bagName.includes(' no other')).map((bagName) => {
    return bagName.replace(/^ /g, '').replace(/ bags.| bag.| bags| bag/, '');
  });
  ...
  let curBag;
  if (!bagMap[container]) {
    curBag = new Bag(container);
    bagMap[container] = curBag;
  } else {
    curBag = bagMap[container];
  }

  for(let j = 0; j < bags.length; j++) {
    let childBag;
    let [count, ...bagName] = bags[j].split(' ');
    bagName = bagName.join(' ');
    if (!bagMap[bagName]) {
      childBag = new Bag(bagName);
      bagMap[bagName] = childBag;
    } else {
      childBag = bagMap[bagName];
    }
    childBag.addParent(curBag);
    for (let x = 0; x < count; x++) {
      curBag.addChild(childBag);
    }
  }
}

const myBag = bagMap['shiny gold'];

const myBagContains = {}

function find(bag) {
  bag.getParentRemoveDuplicated().forEach((p) => {
    myBagContains[p.name] = 1;
    find(p);
  })
}

find(myBag);
console.log('problem answer1', Object.keys(myBagContains).length);

// #problem2
function find2(bag, acc) {
  const tempMap = {};
  ...
  let cur = acc * bag.children.length;
  bag.getChildRemoveDuplicated().forEach((c) => {
    cur += find2(c, tempMap[c.name]) * acc;
  }); 
  return cur;
}

console.log('problem answer2', find2(myBag, 1));