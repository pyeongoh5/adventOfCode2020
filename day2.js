...
const regex = /([0-9]+-[0-9]+) ([a-z,A-Z]){1,1}: ([a-z,A-Z]+)/gi;

function getParams(str) {
  return [...str.matchAll(regex)][0]
}

// # problem1
function countContainCharacter(str, char) {
  let counter = 0;
...
for (let i = 0; i < input.length; i++) {
  const [ none, rangeText, char, password ] = getParams(input[i]);
  const [ min, max ] = rangeText.split('-');

  const count = countContainCharacter(password, char);
  if (count >= min && count <= max) {
    validPasswordsCount++;
  }
}
console.log('problem1 answer', validPasswordsCount);

// # problem2
validPasswordsCount = 0;
for (let i = 0; i < input.length; i++) {
  const [ none, rangeText, char, password ] = getParams(input[i]);
  const [ first, second ] = rangeText.split('-');
  if (password[first - 1] === char ^ password[second - 1] === char) {
    validPasswordsCount++;
  }
}
console.log('problem2 answer', validPasswordsCount);